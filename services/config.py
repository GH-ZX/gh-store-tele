...
CONFIG_DEFINITIONS = {
    ...
    "FIVESIM_API_KEY": {"secret": True, "desc": "5sim SMS activation API key (Bearer token from 5sim.net)."},
    "FIVESIM_API_URL": {"secret": False, "desc": "5sim API base URL. Default: https://5sim.net/v1"},
    "FIVESIM_ENABLED": {"secret": False, "desc": "Enable 5sim virtual number and SMS activation service (true/false)."},
    "FIVESIM_RUB_USD_RATE": {"secret": False, "desc": "5sim RUB to USD exchange rate multiplier (default 0.011)."},
    ...
    "STORE_LOGO_URL": {"secret": False,
                       "desc": "Store brand logo image URL shown in the Mini App header (empty = emoji fallback)."},
    "STORE_HERO_IMAGE_URL": {"secret": False,
                             "desc": "Hero banner image URL for the storefront (empty = built-in local art)."},
    "STORE_EMPTY_PRODUCT_IMAGE_URL": {"secret": False,
                                      "desc": "Fallback product thumbnail URL when a product has no image (empty = built-in local art)."},
    "STORE_DEFAULT_CATEGORY_IMAGE_URL": {"secret": False,
                                         "desc": "Fallback category cover URL when a category has no image (empty = built-in local art)."},
    "PAY_SHAMCASH_LOGO_URL": {"secret": False,
                              "desc": "ShamCash payment method logo URL (empty = built-in local art)."},
    "PAY_SYRIATEL_LOGO_URL": {"secret": False,
                              "desc": "Syriatel Cash payment method logo URL (empty = built-in local art)."},
    "SUPPORT_LINK": {"secret": False, "desc": "Customer support contact link (e.g. https://t.me/ahmedghx)."},
    "SUPPORT_USERNAME": {"secret": False, "desc": "Customer support Telegram username handle (e.g. ahmedghx)."},
    "FAQ_TEXT_AR": {"secret": False, "desc": "Arabic FAQ and store rules text shown to customers in bot and store."},
    "FAQ_TEXT_EN": {"secret": False, "desc": "English FAQ and store rules text shown to customers in bot and store."},
    "HELP_TEXT_AR": {"secret": False, "desc": "Arabic customer support and help guide text."},
    "HELP_TEXT_EN": {"secret": False, "desc": "English customer support and help guide text."},
}
