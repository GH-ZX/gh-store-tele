...
HEALTHCHECK --interval=30s --timeout=5s --start-period=15s --retries=3 \
    CMD python -c "import os, urllib.request; urllib.request.urlopen('http://localhost:5000/health'); (urllib.request.urlopen('http://127.0.0.1:20241/ready') if os.environ.get('TUNNEL_TOKEN') else None)" || exit 1
