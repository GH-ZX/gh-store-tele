...
══════════════════════════
  ...
  root.openAdminUsersPage = (f) => admin().openAdminUsersPage?.(f);
  root.closeAdminUsersPage = () => admin().closeAdminUsersPage?.();
  root.loadAdminUsers = (f, q) => admin().loadAdminUsers?.(f, q);
  root.renderAdminUsers = (u) => admin().renderAdminUsers?.(u);
  root.setAdminUserFilter = (f) => admin().setAdminUserFilter?.(f);
  root.debounceAdminUserSearch = () => admin().debounceAdminUserSearch?.();
  root.clearAdminUserSearch = () => admin().clearAdminUserSearch?.();
  root.toggleAdminUserBan = (id) => admin().toggleAdminUserBan?.(id);
  root.toggleAdminUserReseller = (id) => admin().toggleAdminUserReseller?.(id);
  root.setAdminDiscVal = (v) => admin().setAdminDiscVal?.(v);
  root.submitAdminDiscount = () => admin().submitAdminDiscount?.();
  root.clearAdminDiscount = () => admin().clearAdminDiscount?.();
  root.submitAdminQuickClearDiscount = (id) => admin().submitAdminQuickClearDiscount?.(id);
  root.submitAdminSendMessage = () => admin().submitAdminSendMessage?.();
  ...
  root.loadAdminStuckOrders = () => admin().loadAdminStuckOrders?.();
  root.adminRefundStuckOrder = (id) => admin().adminRefundStuckOrder?.(id);
  ...
  root.openAdminBalanceModal = (id, uname, curr) => admin().openAdminBalanceModal?.(id, uname, curr);
  root.closeAdminBalanceModal = () => admin().closeAdminBalanceModal?.();
  root.openAdminDiscountModal = (id, uname, disc) => admin().openAdminDiscountModal?.(id, uname, disc);
  ...
  root.openAdminMessageModal = (id, uname) => admin().openAdminMessageModal?.(id, uname);
